...
const obtenerPedidosPendientes = async (req, res) => {
  ...
  const pedidosPendientes = await connection.query(
    `select idpedido,fecha,alumno.nombre,aprobado from pedido,alumnocurso inner join alumno where pedido.alumnocurso_id = alumnocurso.idalumnocurso and alumnocurso.alumno_id = alumno.dni and pedido.aprobado=0;`
  );
